namespace TALXIS.TestKit.Bindings.Steps
{
    ...
    public class RelatedGridSteps : PowerAppsStepDefiner
    {
        ...
        {
            var buttonElement = XrmApp.RelatedGrid.GetButtonInFlyout(button);

            buttonElement.Should().NotBeNull("the button in the flyout should exist");
        }
        ...
        {
            Action act = () => XrmApp.RelatedGrid.GetButtonInFlyout(button);

            act.Should().Throw<NoSuchElementException>("the button in the flyout shouldn't exist");
        }
    }
}
